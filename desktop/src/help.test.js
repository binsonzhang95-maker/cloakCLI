...
test("status hint and overlay table stay in sync", () => {
  ...
  assert.match(hint, /1–6/);
  assert.match(hint, /\? help/);
  const table = renderShortcutsTable();
  assert.match(table, /<kbd>1<\/kbd>/);
  assert.match(table, /Fleet/);
  assert.match(table, /Runs/);
});
