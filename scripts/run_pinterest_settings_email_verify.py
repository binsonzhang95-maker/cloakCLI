...
def main() -> int:
    ...
    try:
        ...
        page.screenshot(path=str(art / "settings-runner-03-code-filled.png"))
        print(
            json.dumps(
                {"status": "code_filled", "value_len": len(val), "match": val == code},
                ensure_ascii=False,
            ),
            flush=True,
        )
        cont, cont_sel = find_verify_continue(page)
        wait_continue_enabled(cont, 12000)
        cont.click()
        print(json.dumps({"status": "continue_clicked", "sel": cont_sel}), flush=True)
        ...
        result["status"] = "ok" if verified else "code_submitted_check_ui"
        result["urls"].append(page.url)
        print(json.dumps(result, ensure_ascii=False), flush=True)
